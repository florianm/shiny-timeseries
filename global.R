...
#' Attempt to convert factors of a data.frame into POSIXct dates
#'
#' lapply over a data.frame
try_convert_factors_to_date <- function(x) {
  if(is.factor(x)){
    try(x <- lubridate::parse_date_time(x,
                                        orders=lubridate_formats,
                                        tz="Australia/Perth"), silent=T)
  }
  x
}
